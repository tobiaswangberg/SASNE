function [Y,Z,W,lambda] = SASNE(data,dist,dim)
...
% the graph weight matrix W.
    if ~exist('dist','var')
        get_graph_coords = @get_symbiharmonic_coords;
    elseif dist == 'BHD'
        get_graph_coords = @get_symbiharmonic_coords;
    elseif dist == 'CTD'
        get_graph_coords = @get_CTD_coords;
    end
    if ~exist('dim','var')
        dim = 2;
    end
    ...
    tic;
     Y = tsne(Z,'InitialY',init_Y,'Exaggeration',12,'LearnRate',...
         ...
         statset('TolFun',1e-100),'Algorithm','exact','NumDimensions',dim);
